...
const updateMap = async (city: string, state: string) => {
  try {
    await fetchy("/api/maps", "PATCH", { body: { city: city, state: state } });
  } catch (_) {
    return;
  }
  emit("refreshLocation");
...
<template>
  <form @submit.prevent="updateMap(city, selectedState)">
...
.button-full {
  ...
  width: 100%; /* Ensure buttons take full width of the flex item */
}
</style>
